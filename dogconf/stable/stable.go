// The Go 'fmt' code, except modified to enable stable output order.
//
...
// Implement sorting interfaces for stringified reflect.Value
// values.
type valueSortTuple struct {
	sortKey string
	val     *reflect.Value
}

type valueSortSlice []valueSortTuple

func (vss valueSortSlice) Less(i, j int) bool {
	return vss[i].sortKey < vss[j].sortKey
}

func (vss valueSortSlice) Swap(i, j int) {
	vss[i], vss[j] = vss[j], vss[i]
}

func (vss valueSortSlice) Sort() { sort.Sort(vss) }

func (vss valueSortSlice) Len() int { return len(vss) }

// Sort an array of arbitrary values by a textual rendering
func sortValues(vals []reflect.Value) []reflect.Value {
	// The general approach for this is the schwartzian transform:
	// form tuple "{sortKey, &val}", sorting, and then stripping
	// the sortKey at the end.
	...
	// Project away the sort key and yield the output slice
	out := make([]reflect.Value, len(sortVals))
	for i, _ := range out {
		out[i] = *sortVals[i].val
	}
